...
class MyPromise {
  ...
  #result = undefined; // 初始结果为 undefined
  #handlers = []; // 存储处理函数的数组
  ...
  #run() {
    ...
    while (this.#handlers.length) {
      const { onFullfilled, onRejected, resolve, reject } = this.#handlers.shift(); // 取出一个处理函数
      if (this.#state === FULFILLED) {
        this.#runOne(onFullfilled, resolve, reject); // 如果状态为已完成，运行对应的成功处理函数
      } else {
  ...
  static resolve(value) {
    ...
    const p = new MyPromise((resolve, reject) => {
      _reslove = resolve;
      _reject = reject;
    }); // 创建一个新的 MyPromise 实例，并获取其 resolve 和 reject 函数

    if (p.getIsPromiseLike(value)) {
      value.then(_reslove, _reject); // 如果传入值是类 Promise 对象，调用其 then 方法
    } else {
      _reslove(value); // 否则直接调用 resolve
    }
